...
void setup() {
  ...
  USB.ON();
  USB.printf("-------------------------------\nTest of MQTT-SN:unsubscribe_topicName()\n-------------------------------\n");
  
  xbee802.ON();
}

void loop() {
  int ret = -1, resCon, resCack, resSub, resSack, resUnsub, resUnsuback;
  ...
  resUnsub = mqttSN.unsubscribe_topicName("a/b");
  if(resSub == 1)
    USB.println("---- unsubscribe send");
    
  resUnsuback = mqttSN.recv_unsuback();
  if(resSack == 1)
    USB.println("---- unsuback received");
  
  if (resCon && resCack && resSub && resSack && resUnsub && resUnsuback)
    ret = 1;
